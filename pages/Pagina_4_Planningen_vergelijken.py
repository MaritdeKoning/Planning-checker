...
st.markdown('---')    
st.text("Dit figuur geeft de verhouding tussen de DPRU's en de DRU's weer. De twee lijnen\n geven een indicatie van wat een goede verhouding is. Als de grafiek onder de\n ondergrens zit is het een uitstekende verhouding, als het boven de bovengrens zit\n is het een slechte verhouding. Ertussenin is een goede verhouding.")
f1 = go.FigureWidget()
f1.add_scatter(y=[maximale_DDs, maximale_DDs], x=[0.5,2.5], marker=dict(size=0, color="red"), name = 'bovengrens')
f1.add_scatter(y=[minimale_DDs, minimale_DDs], x=[0.5,2.5], marker=dict(size=0, color="red"), name = 'ondergrens')
...
st.markdown('---')
st.text("Het laatste figuur laat zien hoeveel kWh er in totaal voor elke planning\n teruggeleverd kan worden. Voor elk van de planningen is hiervoor het minimum 0.\n Het maximum wordt berekend op het aantal bussen dat gebruik wordt.")
f4 = go.FigureWidget()
